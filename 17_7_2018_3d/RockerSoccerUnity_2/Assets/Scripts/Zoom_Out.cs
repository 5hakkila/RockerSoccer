       ...
       // print(inputAngle);
        ...
        if (GameObject.Find("Game_Manager").GetComponent<Game_Manager>().releaseBall == true)
        {
            ...
            if (camera.transform.position.y > 0 && slow_Mo_Camera == false)
            {
                ...
                Vector3 target_position = new Vector3(camera.transform.position.x, camera.transform.position.y, -10 - (camera.transform.position.y * 2.5f) - size_Offset);
                //  m_OrthographicCamera.orthographicSize = 4 + camera.transform.position.y + size_Offset;
                Camera.main.transform.position = Vector3.Lerp(camera.transform.position, target_position, 1.7f * Time.deltaTime);
               // camera.transform.position = new Vector3(camera.transform.position.x, camera.transform.position.y, -10 - (camera.transform.position.y *2.0f) - size_Offset); 
            }
        ...
        else if (slow_Mo_Camera == false)
        {
            ...
            Vector3 target_position = new Vector3(camera.transform.position.x, (camera.transform.position.y ), -10 - size_Offset);
            Camera.main.transform.position = Vector3.Lerp(camera.transform.position, target_position, 1.7f * Time.deltaTime);
